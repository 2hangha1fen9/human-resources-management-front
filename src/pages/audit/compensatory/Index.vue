<template>
    <div>
                    ...
                    <template #default="scope">
                        <el-button-group>
                            <el-button type="success" size="small" v-if="scope.row.auditStatus == 1"
                                @click="() => { compensatoryAuditVisible = true; compensatoryapplyid = scope.row.id }">审核</el-button>
                            <el-text type="primary" v-else>已审核&nbsp;</el-text>
                            <el-button type="primary" size="small"
                                @click="() => { compensatoryDetailVisible = true; compensatoryapplyid = scope.row.id }">查看详情</el-button>
                        </el-button-group>
                    </template>
                </el-table-column>
            </el-table>
        ...
        <div class="modal">
            <el-dialog v-model="compensatoryAuditVisible" destroy-on-close title="调休审核"
                style="width: 600px; max-width: 100%">
                <compensatoryAudit :id="compensatoryapplyid"
                    @onClose="() => { compensatoryAuditVisible = false; search() }" />
            </el-dialog>
            <el-dialog v-model="compensatoryDetailVisible" destroy-on-close title="调休申请详情"
                style="width: 600px; max-width: 100%">
                <compensatoryDetail :id="compensatoryapplyid"
...
import compensatoryAudit from './compensatoryAudit.vue'
import compensatoryDetail from '../../attendance/compensatory/compensatorydetail.vue'
...
const compensatoryAuditVisible = ref(false)
const compensatoryDetailVisible = ref(false)
const compensatoryapplyid = ref(0)

const search = async () => {
    try {
        tableLoading.value = true
        let res = await post("/compensatoryapply/QueryCompensatoryListByPage", query)
